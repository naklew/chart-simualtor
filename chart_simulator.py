...
# --- 4. Plotly 차트 생성 함수 (데이터 전달 방식 변경으로 줌 문제 최종 해결) ---
def create_plotly_chart(df, visible_df, trades, state):
    fig = make_subplots(
    ...
    # [핵심 추가] 차트의 x축 범위를 visible_df에 맞게 제한합니다.
    fig.update_xaxes(type='category', range=[visible_df.index[0], visible_df.index[-1]])
    
    fig.update_yaxes(showspikes=True, side='right')
...
try:
    ...
    if df.empty or len(df) < MIN_DATA_PERIOD: st.warning("데이터가 부족합니다.")
        ...
        # [핵심 수정] 함수를 호출할 때 df 전체와 visible_df를 모두 전달합니다.
        # 전체 데이터프레임의 인덱스를 날짜로 설정하여 전달합니다.
        df_with_index = df.copy().set_index('날짜')
        plotly_fig = create_plotly_chart(df_with_index, visible_df, state.get("trade_log", []), state)
