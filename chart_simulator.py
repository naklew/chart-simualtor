# [V6.0] 보조지표(RSI, MACD, 볼린저밴드) 및 기업 기본 정보 기능 추가된 완성본
import streamlit as st
import pandas as pd
import pandas_ta as ta # 보조지표 계산을 위한 라이브러리
...
# --- 1. 초기 설정 ---
st.set_page_config(page_title="실전 차트 시뮬레이터 V6", layout="wide")
st.title("📈 실전형 차트 기반 주식 시뮬레이터 V6")
...
def load_data(ticker, start, end):
    ...
    try:
        ...
        # --- [핵심 수정] 볼린저밴드 기간을 20일로 명확하게 지정합니다. ---
...
def get_fundamental_data(date_str, ticker):
    try:
        # 특정 날짜의 데이터가 없을 수 있으므로, 해당 주 전체의 데이터를 가져와서 가장 가까운 날의 데이터를 사용
...
try:
    ...
    if df.empty or len(df) < MIN_DATA_PERIOD: st.warning("데이터가 부족합니다.")
        ...
        with col1:
            st.subheader(f"📊 {get_stock_name(ticker)} ({ticker}) 차트")
            plotly_fig = create_plotly_chart(visible_df, state.get("trade_log", []))
            st.plotly_chart(plotly_fig, use_container_width=True)

            # 기업 정보 UI를 차트 아래 (col1)로 이동
            st.markdown("---")
            st.subheader("🏢 기업 정보")
            date_str = current_date.strftime("%Y%m%d")
            funda = get_fundamental_data(date_str, ticker)
            f_col1, f_col2, f_col3, f_col4 = st.columns(4) # 4개 컬럼으로 더 보기 좋게
            if funda is not None and not funda.empty:
                f_col1.metric("PER", f"{funda['PER'].iloc[0]:.2f}" if funda['PER'].iloc[0] != 0 else "N/A")
                f_col2.metric("PBR", f"{funda['PBR'].iloc[0]:.2f}" if funda['PBR'].iloc[0] != 0 else "N/A")
                f_col3.metric("EPS", f"{int(funda['EPS'].iloc[0])}" if funda['EPS'].iloc[0] != 0 else "N/A")
                f_col4.metric("BPS", f"{int(funda['BPS'].iloc[0])}" if funda['BPS'].iloc[0] != 0 else "N/A")
            else:
                f_col1.metric("PER", "N/A"); f_col2.metric("PBR", "N/A")
                f_col3.metric("EPS", "N/A"); f_col4.metric("BPS", "N/A")

        with col2:
            st.markdown(f"#### 📅 {current_date.date()} | 종가: {int(current_price):,}원")
            
            # --- [수정] "다음 날로 이동" 버튼을 상단으로 이동 ---
            st.markdown("---")
                ...
                if state["day_index"] < len(df) - 1:
                    ...
                    for order in state["pending_orders"]:
                        ...
                        if order['type'] == 'buy' and next_day['Low'] <= order['price'] and state['cash'] >= order['qty'] * order['price']:
                            ...
                            st.toast(f"✅ [{next_day['날짜'].date()}] {int(order['price']):,}원 매수 체결!"); executed_order_ids.append(order['id'])
                        
                        elif order['type'] == 'sell' and next_day['High'] >= order['price'] and state['holdings']['quantity'] >= order['qty']:
                            revenue = order['qty'] * order['price']; current_avg_price = state['holdings']['avg_price']
                            state['cash'] = float(state['cash'] + revenue); state['holdings']['quantity'] = int(state['holdings']['quantity'] - order['qty'])
                            ...
                            st.toast(f"🚨 [{next_day['날짜'].date()}] {int(sell_price):,}원 스탑로스 체결!"); executed_order_ids.append(order['id'])
                            ...
                            if next_day['Low'] <= stop_price:
                                ...
                                st.toast(f"✅ [{next_day['날짜'].date()}] {int(sell_price):,}원 트레일링 스탑 체결!"); executed_order_ids.append(order['id'])
                    ...
                    state['daily_portfolio_value'].append(portfolio_value)
                    st.rerun()
                else:
                    st.warning("시뮬레이션 기간의 마지막 날입니다.")
            
            # 자산 현황 및 매매 관련 UI는 그대로 아래에 위치
            st.markdown("---")
            st.subheader("💰 자산 현황")
            c1, c2 = st.columns(2)
            c1.metric("현금 잔액", f"{int(state['cash']):,}원")
            c2.metric("평가 금액", f"{int(state['holdings']['quantity'] * current_price):,}원")
            c1.metric("보유 수량", f"{state['holdings']['quantity']}주")
            c2.metric("평균 단가", f"{int(state['holdings']['avg_price']):,}원")
                    ...
                    if st.button("🟩 금액으로 매수", use_container_width=True):
                        if buy_amount > 0 and current_price > 0:
                            ...
                            if buy_qty_from_amount > 0:
                                ...
                                state.update({'holdings': {'quantity': int(total_qty), 'avg_price': float(avg_price)}, 'cash': float(state['cash'] - cost)})
                                state['trade_log'].append({"일자": current_date.date().isoformat(), "유형": "시장가매수", "수량": buy_qty_from_amount, "단가": int(current_price), "금액": int(cost)})
                                st.success(f"{buy_qty_from_amount}주 매수 실행 완료."); st.rerun()
                    ...
                    if st.button("🟥 금액으로 매도", use_container_width=True):
                        if sell_amount > 0 and current_price > 0:
                            ...
                            if sell_qty_from_amount > 0 and sell_qty_from_amount <= state['holdings']['quantity']:
                                revenue = sell_qty_from_amount * current_price; current_avg_price = state['holdings']['avg_price']
                                state['cash'] = float(state['cash'] + revenue); state['holdings']['quantity'] = int(state['holdings']['quantity'] - sell_qty_from_amount)
                                if state['holdings']['quantity'] == 0: state['holdings']['avg_price'] = 0
                                state['trade_log'].append({"일자": current_date.date().isoformat(), "유형": "시장가매도", "수량": sell_qty_from_amount, "단가": int(current_price), "금액": int(revenue), "avg_price_at_sale": float(current_avg_price)})
                                st.success(f"{sell_qty_from_amount}주 매도 실행 완료."); st.rerun()
            ...
            with st.expander("🕰️ 예약 매매 (지정가)", expanded=False):
                limit_buy_price = st.number_input("매수 희망 가격", value=int(current_price * 0.95), step=100)
                limit_buy_qty = st.number_input("매수 수량", min_value=0, step=10, key="limit_buy_qty")
                if st.button(" 예약 매수", use_container_width=True):
                    if limit_buy_price > 0 and limit_buy_qty > 0:
                        state["pending_orders"].append({"id": time.time(), "type": "buy", "price": limit_buy_price, "qty": limit_buy_qty})
                        st.success(f"{int(limit_buy_price):,}원에 {limit_buy_qty}주 매수 예약 완료!"); st.rerun()

                limit_sell_price = st.number_input("매도 희망 가격", value=int(current_price * 1.05), step=100)
                limit_sell_qty = st.number_input("매도 수량", min_value=0, step=10, key="limit_sell_qty")
                if st.button(" 예약 매도", use_container_width=True):
                    if limit_sell_price > 0 and limit_sell_qty > 0 and limit_sell_qty <= state['holdings']['quantity']:
                        state["pending_orders"].append({"id": time.time(), "type": "sell", "price": limit_sell_price, "qty": limit_sell_qty})
                        st.success(f"{int(limit_sell_price):,}원에 {limit_sell_qty}주 매도 예약 완료!"); st.rerun()
                    elif limit_sell_qty > state['holdings']['quantity']: st.warning("보유 수량보다 많이 예약할 수 없습니다.")

            with st.expander("🤖 자동 매매 (손절/익절)", expanded=True):
                st.info("자동 매매 주문은 현재 보유한 모든 수량에 대해 적용됩니다.")
                sl_price = st.number_input("손절 가격 (Stop-Loss)", value=0, step=100)
                if st.button("스탑로스 설정", use_container_width=True):
                    if sl_price > 0 and state['holdings']['quantity'] > 0:
                        state["pending_orders"] = [o for o in state["pending_orders"] if o['type'] not in ['stop_loss', 'trailing_stop']]
                        state["pending_orders"].append({"id": time.time(), "type": "stop_loss", "price": sl_price, "qty": state['holdings']['quantity']})
                ...
                if st.button("트레일링 스탑 설정", use_container_width=True):
                    if ts_percent > 0 and state['holdings']['quantity'] > 0:
                        state["pending_orders"] = [o for o in state["pending_orders"] if o['type'] not in ['stop_loss', 'trailing_stop']]
                        state["pending_orders"].append({"id": time.time(), "type": "trailing_stop", "percentage": ts_percent, "qty": state['holdings']['quantity'], "peak_price": current_price})
                        st.success(f"고점 대비 {ts_percent}% 하락 시 매도 설정 완료!"); st.rerun()

            if state["pending_orders"]:
                st.markdown("---"); st.subheader("📋 예약 주문 목록")
                for order in state["pending_orders"]:
                    col_order, col_cancel = st.columns([4, 1])
                    order_type_map = {"buy": "지정가 매수", "sell": "지정가 매도", "stop_loss": "스탑로스", "trailing_stop": "트레일링 스탑"}
                    order_text = f" - {order_type_map.get(order['type'])}: {order['qty']}주"
                    if order['type'] in ['buy', 'sell', 'stop_loss']: order_text += f" @ {int(order['price']):,}원"
                    elif order['type'] == 'trailing_stop':
                        stop_price = int(order.get('peak_price', current_price) * (1 - order['percentage']/100))
                        order_text += f" ({order['percentage']}%) / 현재가: {stop_price:,}원"
        ...
        st.subheader("🚀 성과 리포트")
        performance = calculate_performance(state, current_price)
        ...
        if state["trade_log"]:
            with st.expander("📒 매매 기록 상세보기"):
                log_df = pd.DataFrame(state["trade_log"]); display_cols = ['일자', '유형', '수량', '단가', '금액']
